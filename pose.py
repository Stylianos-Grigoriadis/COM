...
with open('poseCSVR.csv', 'w', newline='') as csvFile:
  ...
  with mp_pose.Pose(
      ...
      min_tracking_confidence=0.5, model_complexity=2) as pose: #model_complexity can be 0,1 or 2. Higher nr=more accuracy but less FPS
    ...
    while cap.isOpened():
      ...
      angLeft = findAngle(data[now][49],data[now][50],data[now][57],data[now][58],data[now][65],data[now][66])
      angRight = findAngle(data[now][45],data[now][46],data[now][53],data[now][54],data[now][61],data[now][62])
